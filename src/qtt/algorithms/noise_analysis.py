...
def calculate_psd_welch(signal : np.ndarray, sample_rate : float, nperseg : Optional[int] = None) -> Tuple[np.ndarray, np.ndarray]:
    ...
    """
    frequencies, Pwelch = scipy.signal.welch(signal, sample_rate, nperseg = nperseg)
    return frequencies, Pwelch

def plot_psd(frequencies, psd, frequency_unit = 'Hz', signal_unit = 'V', label=None, fig=1):
    """ Plot calculated PSD """
    if fig is not None:
...
def generate_pink_noise(sample_rate: float, number_of_samples : int, A : float = 1, alpha : float= 1):
    """ Generate samples data with 1/f noise
...
def fit_pink_noise(x_data: np.ndarray, y_data: np.ndarray, initial_parameters=None,
                   remove_outliers=False) -> Tuple[Dict[str, Any], Dict[str, Any]]:

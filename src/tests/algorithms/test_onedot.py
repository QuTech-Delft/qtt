...
class TestOneDot(unittest.TestCase):
    ...
    @staticmethod
    def one_dot(fig = None):
        nr_dots = 3
        station = initialize(reinit=True, nr_dots=nr_dots, maxelectrons=2, verbose=0)
        gates = station.gates
        ...
        x = onedotGetBalance(dataset=data, verbose=1, fig=fig)
        results = x[0]
        _, _ = onedotGetBalanceFine(impixel=None, dd=data, fig=fig)
        plot_onedot(results, ds=data, fig=None, verbose=1)

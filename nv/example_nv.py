...
f = lambda plotidx, **kwargs: nv_plot_callback(plotidx, adata, **kwargs); f(0)
pc = pgeometry.plotCallback(func=f, xdata=allData[0], ydata=allData[2])
pc.connect(fig)

pgeometry.tilefigs([fig, 100], [1, 2])
...
import pandas as pd
df = pd.DataFrame((fulldata.T)[:, 0:6], columns=['time', 'gate', 'yellow', 'new', 'gate jump', 'yellow jump'])
...
if 0:
    ...
    plt.figure(44)
    plt.clf()
    df.plot.scatter('gate', 'yellow', s=7, ax=plt.gca())
...
f = lambda plotidx, **kwargs: nv_plot_callback(plotidx, adata, **kwargs); f(0)
pc = pgeometry.plotCallback(func=f, xdata=jumpGate, ydata=jumpYellow)
pc.connect(fig)

pgeometry.tilefigs([None, 100], [1, 2])
...
plt.suptitle('Small jumps', fontsize=16)
for ii in range(16):

...
import pandas as pd
#import seaborn as sns; plt.hold(True)
...
qtt.tools.mkdirc(os.path.join(qcodes.config['user']['nvDataDir'], 'results'))
...
datascalerBase = StandardScaler().fit(Xbase)
x = dataS[:, 4]
y = dataS[:, 5]
...
if 0:
    dbspectral = SpectralClustering(7, gamma=0.2).fit(X)
    core_samples_mask = np.zeros_like(db.labels_, dtype=bool)
    try:
        core_samples_mask[db.core_sample_indices_] = True
    except:
        pass
    labelsSpectral = db.labels_

    encoderSpectral = sklearn.preprocessing.LabelEncoder()
    encoderSpectral.fit(labelsSpectral)

    plt.figure(301)
    plt.clf()
    plt.jet()
    plt.scatter(X[:, 0], X[:, 1], c=labelsSpectral, cmap=cm.jet)

...
def generateOneDotReport(one_dots, xdir, resultsdir, verbose=1):
    ...
    """
    #%
    lst = pgeometry.findfiles(xdir, '.*sweep-2d.*pickle')
    ...
    for idx, od in enumerate(one_dots):
        ...
        pgeometry.plotPoints(od['balancefit'], '--c')
        plt.axis('image')
        ...
        if dd2dhi is not None:
            ...
            if verbose >= 2:
                pgeometry.plotPoints(od['balancepoint'], '.m', color=(.7, 0, .7), markersize=10, label='balancepoint')
            ...
            if 'balancepointfine' in odhi:
                pgeometry.plotPoints(odhi['balancepointfine'], '.m', markersize=14, label='balancepointfine')
        ...
        try:
            ...
            allgatevalues = ddplungermetadata.get('allgatevalues', {})
...
def generateDoubleDotReport(two_dots, resultsdir, tag=None, verbose=1, sdidx=1):
    ...
    lst = pgeometry.findfiles(dxdir, 'scandata-doubledot.*pickle')

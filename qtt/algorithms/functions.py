...
def _cost_gaussian(signal_amp, counts, params):
    ...
    model = gaussian(signal_amp, mean, s, amplitude, offset)
    cost = np.linalg.norm(counts - model)
    return cost

def fit_gaussian(signal_amp, counts, maxiter=None, maxfun=5000, verbose=1, par_guess=None):
    ...
    Returns:
        ...
        par_guess (array): initial guess for the fit parameters, either the ones give to the function, or generated by the function: [mean, s, amplitude, offset]
    """
    def func(params): return _cost_gaussian(signal_amp, counts, params)
    maxsignal = np.percentile(signal_amp, 98)
    minsignal = np.percentile(signal_amp, 2)
    if par_guess is None:
        amplitude = np.max(counts)
        s = (maxsignal - minsignal) * 1 / 20
        mean = minsignal + 1 / 2 * (maxsignal - minsignal)
        offset=np.min(counts)
        par_guess = np.array([mean, s, amplitude, offset])
    par_fit = scipy.optimize.fmin(func, par_guess, maxiter=maxiter, maxfun=maxfun, disp=verbose >= 2)

    result_dict = {'parameters initial guess': par_guess}
...
def fit_double_gaussian(signal_amp, counts, maxiter=None, maxfun=5000, verbose=1, par_guess=None):
    ...
    Returns:
        ...
        par_guess (array): initial guess for the fit parameters, either the ones give to the function, or generated by the function: [A_dn, A_up, sigma_dn, sigma_up, mean_dn, mean_up]

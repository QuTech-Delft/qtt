...
class slopeClick():
    def __init__(self, plot):
        ...
        self.fig.axes[1].autoscale(enable=False)
        self.idx = 1
    ...
    def __call__(self, event):
        ...
        if self.idx == 1:
            ...
            print('point1: [%.2f, %.2f]' % (event.xdata, event.ydata))
            self.drawpoint1 = self.draw(self.point1,'ro')
        elif self.idx == 2:
            ...
            self.line[0].remove()
            self.drawpoint1[0].remove()
            self.drawpoint2[0].remove()
            self.idx = 0
            plt.draw()
            
        self.idx += 1
        
    def draw(self, point, style):
        drawpoint = plt.plot([point[0]], [point[1]], style, figure=self.fig)
        plt.draw()
        return drawpoint
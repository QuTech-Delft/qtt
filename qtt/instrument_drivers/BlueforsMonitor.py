...
class BlueforsApp():
    ...
    def print_usage(self, status=0):
        ...
        print('    [-h, -?, --help]   print command line usage information')
    ...
    def set_portnumber(self, port: str):
        try: self._port_ = int(port)
        except ValueError: self.print_usage(-3)
    ...
    def main(self, argv):
        if len(argv) < 2 : self.print_usage(-1)
        try: options, arguments = getopt.getopt(argv[1:], BlueforsApp._short_options_, BlueforsApp._long_options_)
        except getopt.GetoptError: self.print_usage(-2)
        for option, argument in options:
            if option in ('-?', '-h', '--help'): self.print_usage()
            elif option in ("-u", "--un"): self.set_username(argument)
            elif option in ("-p", "--pw"): self.set_password(argument)
            elif option in ("-d", "--dir"): self.set_directory(argument)
            elif option in ("-n", "--port"): self.set_portnumber(argument)
        self.print_settings()
        self.check_directory()
        FridgeDataSender(self._directory_, port=self._port_, user=self._username_, password=self._password_)

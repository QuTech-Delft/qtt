...
#        last_byte=chr(value&255)
    ...
    def read_raw_bytes(self, size=None):
        ...
        if not len(chunk)==size:
            raise Exception ('Visa received incorrect number of bytes: asked %d, received %d' % (size, len(chunk)) )
    ...
    def get_ch1_data(self,address=2, checkdone=True, buf=True):
        ...
        if buf:
#            readresultbuf=self.read_raw_bytes(size=3*Npoint) # breaks at value 10 because it is interpreted as '\n'
            readresultbuf=b''
    ...
    def get_ch2_data_old(self,address=6, checkdone=True):
        '''
        Reads signed data out of the FPGA
        '''
        index=self.get_ch2_datapoint_num()

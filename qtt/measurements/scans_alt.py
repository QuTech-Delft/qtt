...
def scan1Dfast(station, scanjob, location=None, liveplotwindow=None, verbose=1):
    ...
    data = measuresegment(waveform, Naverage, station, minstrhandle, read_ch,
                          period=period, sawtooth_width=waveform['width' ], waittime=Naverage*period)
...
def measuresegment(waveform, Naverage, station, minstrhandle, read_ch, mV_range=5000, period=None, sawtooth_width=None, waittime=0):
    try:
    ...
    if isfpga:
        ...
        devicedata = minstrhandle.readFPGA(ReadDevice=ReadDevice, Naverage=Naverage, waittime=waittime)
...
def scan2Dfast(station, scanjob, location=None, liveplotwindow=None, plotparam='measured', diff_dir=None, verbose=1):
    ...
    data = measuresegment(waveform, Naverage, station, minstrhandle, read_ch, period=period, sawtooth_width=waveform['width' ], waittime=period*Naverage)
    if len(read_ch) == 1:
    ...
    if type(stepvalues) is np.ndarray:
        ...
        for idsub, sub_param in enumerate(stepdata['param'].params[1:]):
            data_arr_tmp = DataArray(parameter=sub_param, name=sub_param.name, array_id=sub_param.name, unit='dBm', set_arrays=(alldata.arrays[stepdata['param'].params[0].name],), is_setpoint=False, preset_data=stepvalues[:, idsub+1])
            alldata.add_array(data_arr_tmp)
    else:
    ...
    for ix, x in enumerate(stepvalues):
        if type(stepvalues) is np.ndarray:
            tprint('scan2Dfast: %d/%d: setting %s to %.3f and %s to %.3f' % (ix, len(stepvalues), stepdata['param'].params[0].name, x[0], stepdata['param'].params[1].name, x[1]), dt=.5)
        else:
...
#            stepvalues.set(x)
        ...
        data = measuresegment(waveform, Naverage, station, minstrhandle, read_ch, period=period, sawtooth_width=waveform['width' ], waittime=Naverage*period)
    ...
    if 'vec' in scanjob['scantype']:
        for param in scanjob['phys_gates_vals']:
            parameter = gates.parameters[param]
            if type(stepvalues) is np.ndarray:
                arr = DataArray(name=parameter.name, array_id=parameter.name, label=parameter.label, unit=parameter.unit, preset_data=scanjob['phys_gates_vals'][param], set_arrays=(alldata.arrays[stepdata['param'].params[0].name], alldata.arrays[sweepvalues.parameter.name]))
            else:
                arr = DataArray(name=parameter.name, array_id=parameter.name, label=parameter.label, unit=parameter.unit, preset_data=scanjob['phys_gates_vals'][param], set_arrays=(alldata.arrays[stepvalues.parameter.name], alldata.arrays[sweepvalues.parameter.name]))
            alldata.add_array(arr)

# -*- coding: utf-8 -*-
"""
Created on Mon May 29 13:14:25 2017
...
# -*- coding: utf-8 -*-
"""
Created on Thu May 26 12:04:15 2016
...
sq_pulse_marker = pulse.SquarePulse(channel='ch1_marker1', name='A square pulse on MW pmod')
...
def create_ttrace(ttrace, pulsars, name='ttrace', verbose=1, awg_map=None, markeridx=1):
    ...
    for pi, ttrace_element in enumerate(ttraces):
        add_fill(ttrace_element, fillperiod=period0, channels='all', tag='fillx', verbose=0)
    ...
    endtime = lasttime(ttrace_element)  # ttrace_element.pulses[lp].effective_stop()
    ...
    for ii, tt in enumerate(traces):
        ...
        ttrace['tracedata'] += [{'start_time': start_time + alpha * period, 'end_time': start_time + (1 - alpha) * period}]
        ...
        for g, a in tt:
            ...
            # endtime
        lp = lastpulse(ttrace_element)
        add_fill( ttrace_element, refpulse=lp, tag='fill%d' % ii, fillperiod=fillperiod, refpoint='end')
        lp = lastpulse(ttrace_element)
        endtime = lasttime(ttrace_element)  # endtime

    startx = lasttime(ttrace_element)
    for pi, ttrace_element in enumerate(ttraces):
        add_fill(ttrace_element, fillperiod=period0, start=startx, tag='lastfill')

    if 'awg_mk' in awg_map:
        pi, ci, mi = awg_map['awg_mk']
        pulsar = pulsars[pi]
        ttrace_element = ttraces[pi]
        # FIXME: lasttime(ttrace_element)-markerperiod-awg_delay
...
def add_fill(awg_element, tag, channels =  None, refpulse=None, fillperiod=1e-7, start=0, refpoint='start', verbose=0):
    ...
    sq_pulse = pulse.SquarePulse(channel=channels[0], name='A dummy square pulse')
    for ii, ch in enumerate(channels):
        R = 0
        px = pulse.cp(sq_pulse, amplitude=R, length=fillperiod, channel=ch, channels=[])
        name = str(tag) + '%s' % ch
        awg_element.add(px,
                           name=name, start=start, refpulse=refpulse, refpoint=refpoint)
        if verbose:
            print('add_fill: ch %s: name %s: amplitude %s' % (ch, name, px.amplitude))
        if refpulse is None:
            refpulse = name
...
def show_element(elmnt, fig=100, keys=None, label_map=None):
    ...
    #tt,xx=elmnt.normalized_waveforms()
    tt, xx=elmnt.waveforms()
    if fig is not None:
        qtt.pgeometry.cfigure(fig); plt.clf();
